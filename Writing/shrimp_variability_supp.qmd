...
#| fig-cap: "Posterior predicted distribution (PP Length) and observed data for base model. Each tick on the x-axis is one month; solid vertical lines separate years. Data replicates are across state areas and years."
#| cache: true
...
#| fig-cap: "Growth model process errors by age. Each box represents one month. Numbers are the average number of shrimp measured each month per state area per year. They are offset from the process errors to emphasize that each process error relies on measurments during two consecutive months. Adjacent wintertime distributions look identical because there is no data collected during those months (i.e., blank spaces indicate sample sizes of zero)."
#| fig-width: 7.5
