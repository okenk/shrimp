...
```{r}
#| label: tbl-mod-est
#| tbl-cap: Estimates of global (non-hierarchical) parameters for base model (no environmental covariates). Mean is the posterior mean, sd is the standard deviation of the parameter estimate, n_eff is effective number of MCMC draws, 2.5% and 97.5% cover the 95% credible interval of the parameter, 50% is the posterior median, and Rhat is a convergence diagnostic that compares variability within and among chains and should be close to one.
#| cache: true
...
![Map of the state statistical areas (courtesy of ODFW)](../figures/shrimp_areas.png){#fig-map width="4in"}

{{< pagebreak >}}

```{r}
#| label: fig-obs-data
#| fig-cap: "Length-at-age of pink shrimp by recruitment year and state area. State area number increases with latitude (i.e., area 32 borders Canada and area 12 is in California; @fig-map)."
#| fig-height: 3.5
#| fig-width: 6.5
...
```{r}
#| label: fig-base
#| fig-cap: "Revenue-per-recruit as a function of fishing mortality rates for five fishery opening dates and three natural mortality scenarios. Dotted lines are at the low and high range of estimated fishing mortality in the fishery. Uncertainty ribbon is the 95% credible interval, accounting for uncertainty from the growth model estimates, of the line for April 1 (status quo)."
#| fig-height: 3.5
#| fig-width: 6.5

early_opening <- rpr_base_summary |>
  filter(opening == 1) |>
...
```{r}
#| label: fig-years
#| fig-cap: "Revenue-per-recruit for different potential season opening dates based on estimated size-at-recruitment each year at low (0.03), average (0.1) and high (0.2) fishing mortality rates at the mean location."
#| fig-height: 3.5
#| fig-width: 6.5
