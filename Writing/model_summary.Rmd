...
There were also two simpler models fit. One with the initial size equation as:
...
The second variation did estimate the "mega-region" effects each year but did not estimate the $\gamma_r$'s or $\sigma_r$. Although there is clear evidence of lengths decreasing as you go up the coast, this phenomenon can still be captured somewhat with the "mega-region" effects. This variation was also a test to see if there was a way to improve MCMC mixing (see below).
...
The model is fit in Stan as a multivariate autoregressive state-space model. For the linear model, the global mean is subtracted from all lengths to improve numerical stability. Each year class is essentially a different "state", and each area represents a different observation process of the same state. Four chains are run for 5000 iterations, and the final 2500 iterations of each chain are retained. This is probably not sufficient for formal estimation, but seemed good enough for a first pass at major patterns.

The mixing on all parameters associated with initial size is quite poor. For most of these parameters, effective sample size is \<10% of the total sample size. BUT R hat is close to one. I can try running it longer, but that will come back to bit us when we do variable selection...

Effective sample sizes for these parameters are:
...
mod.list <- list(Simple = mod.simple, `Mega-region` = mod.mega.region)
mcmc.res.list <- sapply(mod.list, function(x)
  rstan::summary(x, pars = # select initial size paramters only
          c(grep('x0', names(x@sim$samples[[1]]), value = TRUE), 
            grep('area', names(x@sim$samples[[1]]), value = TRUE)),
          probs = c())$summary[,c('n_eff','Rhat')] # aggregated chains
)

mcmc.res.arr <- array(dim = c(5, 
                              ncol(mcmc.res.list[[1]]),
                              length(mod.list)),
                      dimnames = list(
                        c('$\\mu_{L1}$',
                          '$\\sigma_{L1}$',
                          'Mean $\\varepsilon_{1,c,r}$', 
                          'Mean $\\gamma_r$', 
                          '$\\sigma_r$'),
                        c('ESS', 'Rhat'),
                        names(mod.list)
...
mcmc.res.arr[1:2,,] <- sapply(mcmc.res.list, function(x) x[1:2,])
mcmc.res.arr[3,,] <- sapply(mcmc.res.list, function(x)
  apply(x[grep('x0\\[', rownames(x)),], 2, max)
)
mcmc.res.arr[4,,] <- sapply(mcmc.res.list, function(x)
  apply(x[grep('area_offset', rownames(x)),], 2, mean)
)

mcmc.res.arr[5,,] <- sapply(mcmc.res.list, function(x)
                     x[grep('sigma_area', rownames(x)),]
                     )

round(mcmc.res.arr[,'ESS',], 0) %>%
  knitr::kable()

# total sample size = 10k

```

For $\varepsilon$ and $\gamma$ the effective sample sizes reported are average effective sample sizes across all years/areas/mega-regions (as appropriate). When reported as the percent of total sample size it is clear how abysmal the mixing is:

```{r, echo=FALSE}
knitr::kable(
  mcmc.res.arr[,'ESS',] / with(mod.simple@sim, (iter-warmup)*chains),
  digits = 3, drop0trailing = FALSE)

```

*However*, the chains to appear to have converged to a stationary distribution, as evidenced by $\hat{R}$:

```{r, echo=FALSE}

knitr::kable(mcmc.res.arr[,'Rhat',], digits = 3, drop0trailing = FALSE)
```

(Although the $\hat{R}$ values are averaged for $\varepsilon$ and $\gamma$, there is very little variability, and the maximum $\hat{R}$ is close to the mean.)

**Question**: What is an acceptable effective sample size?
...
mod.summary <- summary(mod.mega.region, pars = c('x0_mean', 'sigma_x0', 'sigma_area', 'U_season', 'U', 'B', 'sigma_process', 'sigma_obs'))$summary
