...
xx <- lengths %>%
  ...
  scale_color_gradient2(
    limits = range(lengths$Year),
    midpoint = mean(range(lengths$Year)),
    low = pal[1],
    mid = pal[3],
    high = pal[6]
  ) +
...
anova.tibble <- summary(
  mod,
  pars = c('sigma_x0', 'sigma_area', 'sigma_process', 'sigma_obs')
)$summary[, c(4, 6, 8)] %>%
  as_tibble %>%
  mutate(
    param = c(
...
anova.tibble %>%
  ggplot() +
  geom_segment(
    aes(x = param, xend = param, y = `2.5%`, yend = `97.5%`),
    lwd = 1.2
  ) +
  ...
  ylim(0, 1.25) +
  scale_color_manual(
    values = inauguration::inauguration('inauguration_2021', n = 4)
  )
ggsave(
  filename = here('Figures/AFS/anova.png'),
  device = png,
  width = 7,
  height = 5,
  units = 'in',
...
pal <- PNWColors::pnw_palette('Shuksan2', 5)
names(pal) <- levels(rpr_base_summary$opening_fct)

p <- rpr_base_summary |>
  filter(M.multiplier == 1, opening == 1) |>
  ggplot() +
  geom_ribbon(
    aes(x = FF, ymin = low, ymax = high),
...
(p +
  ...
  (rpr_base_summary %>% #update to include all five opening dates
    filter(M.multiplier == 1))
ggsave(
  here('Figures/AFS/rpr3.png'),
  width = 9,
  height = 5,
  device = png,
  units = 'in',
...
(p +
  ...
  #   ylim(0, 1.5)) %+%
  filter(rpr_by_area_summary, area == '20' | area == '32')
ggsave(
  here('Figures/AFS/rpr_area2.png'),
  width = 9,
  height = 5,
  device = png,
  units = 'in',
...
base_mod <- mod
env_mod <- readRDS(here('Code/covars/model_fit_sst.rds'))

draws_base <- list_of_draws
...
env_pred <- purrr::map_dfc(
  covar_all_yr$sst,
  ~ tibble(
    as.vector(draws_env$x0_mean) +
      as.vector(draws_env$covar_par) * .x
  )
) %>%
  setNames(covar_all_yr$year) %>%
  mutate(constant = as.vector(draws_base$x0_mean), mcmc_iter = 1:n()) %>%
  pivot_longer(
    cols = -c(constant, mcmc_iter),
    names_to = 'year',
    values_to = 'length_env'
  )
...
x0_comparison <- apply(draws_base$x0, c(1, 3), identity) %>%
  ...
  as_tibble() %>%
  mutate(
    across(everything(), ~ .x + as.vector(draws_base$x0_mean)),
    mcmc_iter = 1:n()
  ) %>%
  pivot_longer(-mcmc_iter, names_to = 'year', values_to = 'length_obs') %>%
  full_join(env_pred) %>%
  mutate(
    const_error = constant - length_obs,
...
p <- x0_comparison %>%
  ...
  scale_color_manual(
    values = inauguration::inauguration('inauguration_2021', n = 3),
...
ggsave(
  here('Figures/AFS/density_ex1.png'),
  width = 9,
  height = 5,
  device = png,
  units = 'in',
...
p %+%
  ...
  xlim(min(p$data$estimate) + mean(y.vec), max(p$data$estimate) + mean(y.vec))
ggsave(
  here('Figures/AFS/density_ex2.png'),
  width = 9,
  height = 5,
  device = png,
  units = 'in',
...
p
ggsave(
  here('Figures/AFS/density_ex3.png'),
  width = 9,
  height = 5,
  device = png,
  units = 'in',
...
p <- x0_comparison %>%
  ...
  ggplot() +
  geom_violin(
    aes(x = year, y = improvement_pct, fill = year != 2003),
    col = 'gray20'
  ) +
  ...
  scale_fill_manual(
    values = c(
      inauguration::inauguration('inauguration_2021', n = 3)[3],
      'gray80'
    )
  )
